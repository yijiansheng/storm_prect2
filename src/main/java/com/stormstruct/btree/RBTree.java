 ...
 * Author: changdalin
 * Date: 2017/9/27
 * Description: https://zhuanlan.zhihu.com/p/24367771
 **/
...
class RBTree<T extends Comparable<T>> {
    ...
    private void fixRemove(RBTreeNode<T> node,boolean isParent){
        ...
        while(!isRed && !isRoot(cur)){
            ...
            if(sibling.isRed() && !isLeft){//case 1
                ...
                /**
                 * date:2017/9/27
                 * description:必须满足兄弟节点是黑的，并且兄弟节点的子节点不全部是黑，有个红的，
                 */
                //兄弟节点是黑的，兄弟节点的右子节点是红的
                /**
                 * date:2017/9/27
                 * description:所要做的操作是:
                 * 左旋上层节点
                 * cur舍弃本来的指针，指向root，丢弃这个节点
                 */
    ...
    private void fixInsert(RBTreeNode<T> x){
        ...
        /**
         * date:2017/9/27
         * description:这个修复的过程，是个while循环。
         * 出口是  上层节点是黑，就不用修复了，因为插入的是红的
         */
    ...
    private void rotateLeft(RBTreeNode<T> node){
        //找右节点
        //左旋转，是node的right节点上升到node的位置
        // node.right换成right.left
        // right.left换成node
        // 上层将node位置换成right
        ...
        RBTreeNode<T> parent = node.getParent();
        //node = 40 node重新设置right
        node.setRight(right.getLeft());
        setParent(right.getLeft(),node);
        //right 将left设置成node ,right的右节点不用动
        right.setLeft(node);
